    ...
    async function resolvePosition(position) {
      ...
      let results = await queryApi(position);
      console.log(results)
      if (results) {
        ...
        for (var i = 0; i < state_input.options.length; i++) {
          if (state_input.options[i].text === results.address.state)
            state_input.selectedIndex = i;
        }
      }
    }
