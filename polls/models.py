...
class Question(models.Model):
    ...
    poll_opens = models.DateTimeField(_("poll opens"), default=timezone.now, blank=True)
    poll_closes = models.DateTimeField(_("poll closes"), default=two_weeks_hence, blank=True)
    ...
    def can_vote(self, member):
        if self.poll_opens <= timezone.now() <= self.poll_closes:
            return member.get_active_scouts().count() - self.vote_set.filter(family=member.family).count()
...
class Vote(models.Model):
    family = models.ForeignKey('membership.Family', on_delete=models.CASCADE)
    question = models.ForeignKey(Question, on_delete=models.CASCADE)
